...
export async function GET(req: NextRequest) {
  ...
  const slippagePercent = searchParams.get("slippagePercent") ?? "0.5";
  ...
  try {
    ...
    return NextResponse.json({
      base58Transaction: data.tx?.data ?? "",
      routerResult: data.routerResult,
      minReceiveAmount: data.tx?.minReceiveAmount ?? "0",
    });
  } catch (err) {
    console.error("[Swap Instruction API] Error:", err);
    return NextResponse.json({ error: "Failed to fetch swap instruction" }, { status: 500 });
  }
}
